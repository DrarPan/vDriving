...
vmode=0 #0 for linear speed, 1 for angular velocity
...
kernel1=tf.Variable(tf.truncated_normal([11,11,3,64],dtype=tf.float32,stddev=0.1),name='weights1')
...
kernel3=tf.Variable(tf.truncated_normal([3,3,192,128],dtype=tf.float32,stddev=0.1),name='weights3')
...
with tf.Session() as sess:
	f=open('./performance/model_linear_3conv_11_5_3_linearx.txt','w')
	c_bar=0
	...
	for i in range(max_iteration):
		...
		c, p, _ =sess.run([cost,predict,train_step],feed_dict={ImageInput: feedimage, VelInput: feedlabel, keep_prob: 0.6})
		c_bar+=c
